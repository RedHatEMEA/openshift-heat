...
keystone_setup() {
    ...
    SERVICE_TENANT=$(get_data 2 services 1 keystone tenant-list)
    die_if_not_set $LINENO SERVICE_TENANT "Fail to get services tenant 'keystone tenant-list' "
    SERVICE_PASSWORD=${SERVICE_PASSWORD:-$OS_PASSWORD}
    SERVICE_HOST=${SERVICE_HOST:-openshift}
    ...
    OPENSHIFT_USERNAME="openshift"
    OPENSHIFT_USERID=$(get_user $OPENSHIFT_USERNAME)
    die_if_not_set $LINENO OPENSHIFT_USERID "Fail to get user for $OPENSHIFT_USERNAME"
    echo OPENSHIFT_USERID $OPENSHIFT_USERID
    add_role $OPENSHIFT_USERID $SERVICE_TENANT $ADMIN_ROLE $OPENSHIFT_USERNAME

    OPENSHIFT_SERVICE=$(get_service openshift paas \
                      "OpenShift PaaS Service")
    add_endpoint $OPENSHIFT_SERVICE "https://$SERVICE_HOST/broker/rest"
}
